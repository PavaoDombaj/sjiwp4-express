...
router.post(
  ...
  function (req, res, next) {
    ...
    const checkUserTeam = db.prepare("SELECT id FROM teams WHERE id_user = ? AND id_competition = ?;");

    const userTeam = checkUserTeam.get(userId, competitionId);

    if (!userTeam) {
      // Dobivanje maksimalnog id_team iz tablice teams
      const getMaxTeamId = db.prepare("SELECT MAX(id_team) AS maxTeamId FROM teams;");
      const maxTeamId = getMaxTeamId.get().maxTeamId || 0; // Ako je maxTeamId null, postavljamo ga na 0

      // Novi id_team će biti maksimalni id_team + 1
      const newTeamId = maxTeamId + 1;

      // Umetanje novog tima s generiranim id_team
      const stmt = db.prepare(
        "INSERT INTO teams (id_user, id_competition, name, score, id_team) VALUES (?, ?, ?, 0, ?);"
      );
      const insertResult = stmt.run(userId, competitionId, teamName, newTeamId);
...
router.get("/delete/:id", adminRequired, function (req, res, next) {
  ...
  let deleteResultCompetitors; // Definicija deleteResultCompetitors promenljive izvan if/else blokova

  if (competitionInfo.team_size === 1) {
    const stmtCompetitors = db.prepare(
      "DELETE FROM competitors WHERE id_competition = ?;"
    );
    deleteResultCompetitors = stmtCompetitors.run(req.params.id);
  } else {
    const stmtCompetitors = db.prepare(
      "DELETE FROM teams WHERE id_competition = ?;"
    );
    deleteResultCompetitors = stmtCompetitors.run(req.params.id);
  }
...
// GET /competitions/delete/:id
router.get("/team/delete/:id", adminRequired, function (req, res, next) {
  // do validation
  const result = schema_id.validate(req.params);
  if (result.error) {
    throw new Error("Neispravan poziv");
  ...
  const stmt = db.prepare("DELETE FROM teams WHERE id_team = ?;");
  const deleteResult = stmt.run(req.params.id);
